...
const NewPassword = ({setStage}) => {
    ...
    const handleChange = ({target}) => {
        if (target.name === "verifiedPassword") {
            setValidatePassword(target.value === password.new_password ? true : false)
        }
        setPassword({
            ...password,
    ...
    const handleSubmit = async (e) => {
        ...
        const errores = valPassword({new_password: password.new_password},validatePassword);
        if (Object.keys(errores).length === 0) {
                let set_password = await actions.newPassword({new_password: password.new_password})
                console.log(store.status)
                if (store.alert.msg == "Please fill new password") {
                    console.log("LLene los campos de contraseña")  
                }
            setErrors(errores);
            setStage('login')
        ...
        <div className="w-75 m-auto">
            ...
            <div>
                <div className="col-xxl-12 py-2">
                    ...
                    <input name="new_password" type="password" className="form-control" id="validationCustom01" placeholder="Contraseña " required
                        onChange={
                            e => handleChange(e)
                        }/> {
                    errors.password && <p> {
                ...
                <div className="col-xxl-12">
                    ...
                    <input name="verifiedPassword" type="password" className="form-control" id="validationCustom02" placeholder=" Contraseña"
                        onChange={
                            e => handleChange(e)
                        }
                        required/> {
